...
public:
    ...
    void PrintLIFO(int IDregion, int num) {
        for (Customer* p = this->tail; p != nullptr && num > 0; p = p->prev) {
            solution << IDregion << '-' << p->CustomerResult << '\n';
            num--;
        }
    }
    ...
    void DeleteCustomer(int time, int num) {
        if (num >= region->getSize()) {
            ...
            for (int i = 0; i < num; i++) {
                ...
                solution << p->CustomerResult << '-' << ID << endl;
                delete p;
                NumOfCustomer--;
            }
...
protected:
    ...
    void InorderPrint(Node* root) {
        ...
        if (root->key == '#') solution << root->value << '\n';
        else solution << root->key << '\n';
...
protected:
    ...
    vector<pair<const char,int>> CaesarEncode() {
        ...
        /*sort(res.begin(), res.end(), [](pair<char, int>& a, pair<char, int>& b)->bool {
            if (a.second == b.second) {
                if ((a.first <= 'z' && b.first <= 'z' && a.first >= 'a' && b.first >= 'a') 
                    || (a.first >= 'A' && b.first >= 'A' && a.first <= 'Z' && b.first <= 'Z')) {
...
inline void Test() {
 ...
 /*   Restaurant* a = new Restaurant();
    ...
    cout << a->Encode("QAONxxbPvbheiYrtwGPocYzSJMUYhupVdGqqwoWnUfytMcktnOMeSpEuWMIenrJBzTZMelN");
    //a->InorderPrint();
    delete a;
