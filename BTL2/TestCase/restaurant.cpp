...
static vector<pair<char, int>> freq_LAPSE;
static int result_LAPSE;                          
static string root_LAPSE;

static vector<int> number_KOKUSEN;
static vector<string> root_before_KOKUSEN;
static vector<string> root_after_KOKUSEN;

static vector<pair<int, int>> Heap_before_KEITEIKEN;
static vector<pair<int, int>> Heap_after_KEITEIKEN;

static vector<string> customers_before_KEITEIKEN;
static vector<string> customers_after_KEITEIKEN;
...
protected:
    ...
    //static vector<vector<int>> Combiantion(int n, int k) {
...
public:
    ...
    //MOI THEM VAO
    void ToStringBSTRec(Customer* root, string& s) {
        if (!root) { s += "NULL"; return; }
        s += to_string(root->CustomerResult); 
        s += "(";
        ToStringBSTRec(root->left, s);
        s += ","; 
        ToStringBSTRec(root->right, s);
        s += ")";
    }

    string ToStringBST() {
        string s = "";
        ToStringBSTRec(root, s);
        return s;
    }
};
...
public:
    ...
    //// xoa gian diep FIFO   
    //void DeleteSpyPeople() {
    //    int num = tree->NumOfSpyNode();
    //    if (num > 0 && num >= numOfCustomer) tree->Clear();
    //    else {
    //        for (int i = 0; i < num; i++) {
    //            Delete();
    //        }
    ...
    //MOI THEM VAO
    void DeleteSpyPeopleONGTIEN() {
        int num = tree->NumOfSpyNode();
        number_KOKUSEN.push_back(num);

        root_before_KOKUSEN.push_back(tree->ToStringBST());
...
public:
    ...
    void CreateTreeONGTIEN(const vector<pair<const char, int>>& arr) {
        priority_queue<Node*, vector<Node*>, CompareNode> q;
        ...
        //MOI THEM VAO 
        if (!root_LAPSE.empty()) root_LAPSE = "";
        if (root->key == '#') {
            root_LAPSE = "[" + to_string(root->value) + "]"; 
            if (root->left || root->right) {
                root_LAPSE += "(";
                if (root->left) root_LAPSE += to_string(root->left->value);
                else root_LAPSE += "NULL";
                root_LAPSE += ",";
                if (root->right) root_LAPSE += to_string(root->right->value);
                else root_LAPSE += "NULL";
                root_LAPSE += ")";
            }
        }
...
protected:
    ...
    // HAM FIX BUG 
    vector<pair<const char, int>> CaesarEncodeONGTIEN() {
        map<char, int> mp;
        for (char& x : this->name) mp[x]++;
        
        //HAM THEM VAO 
        if (!freq_LAPSE.empty()) freq_LAPSE.clear();
        for (auto& it : mp) {
            freq_LAPSE.push_back({ it.first, it.second });
        }
        sort(freq_LAPSE.begin(), freq_LAPSE.end());
...
public:
    ...
    int Encode(string name) {
        ...
        vector<pair<const char, int>> arr = CaesarEncodeONGTIEN();
...
inline void Test() {
    ...
    //node->InsertHashRegion(new Customer(1, 10));
    //node->InsertHashRegion(new Customer(2, 20));
    //node->InsertHashRegion(new Customer(8, 5));
    //node->InsertHashRegion(new Customer(9, 7));
    //node->InsertHashRegion(new Customer(10, 10));
    //node->InsertHashRegion(new Customer(11, 20));
    //node->PrintStackTime();
    //node->PrintPeople();
    //node->Delete();
    //node->PrintPeople();
    ...
    a->Print();
    cout << "---------------------------" << endl;
    a->InsertCustomer(4, new Customer(1, 10));
    a->InsertCustomer(4, new Customer(2, 80));
    a->InsertCustomer(4, new Customer(3, 70));
    a->InsertCustomer(4, new Customer(4, 50));
    a->InsertCustomer(6, new Customer(8, 60));
    a->InsertCustomer(6, new Customer(10, 20));
    a->InsertCustomer(6, new Customer(11, 30));
    a->InsertCustomer(6, new Customer(13, 140));
    a->InsertCustomer(2, new Customer(18, 201));
    a->InsertCustomer(2, new Customer(19, 12));
    a->InsertCustomer(7, new Customer(20, 78));
    ...
    cout << a->Encode("XTFTJWxvhpNImvVDNSgajBvOcrjMFVwBenkBNOGpqmuQFyjPbqiGSapMpQjOLMCSrPYspRJJmNqNJuOZGNTIXWyfrsffflDktHoixFECZwbbEebPGupnHbwJOGXQYwvRcAyKyMPpfnCxyVSyBdSP");
    //a->InorderPrint();
    delete a;
